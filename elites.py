...
def main(infile, decay, threshold, granularity, interval, outfile):
    ...
    logging.info('Determining effective decay')
    effective_decay = decay
    if granularity == 'M':
        effective_decay= decay**(30)
    if granularity == 'Y':
        effective_decay= decay**(365.25)       
    if granularity != 'M' and granularity != 'Y' and granularity != None:
        time_delta = pd.to_timedelta('1'+granularity)
        effective_decay= decay**(time_delta.total_seconds()/(24*3600))
        
    #Determine interval if applicable. 
    ...
    logging.info('Writing temporal output file: ' + first_temporal_csv)
    f_temp_output = open(first_temporal_csv, 'w', encoding="utf-8")
    f_temp_output.write("created_at,author_name,author_profile\n")

    #Fill author_profile dictionary and write date and user in temporal file.
    ...
    #Create temporal file wit profiles and unique dates
    second_temporal_csv = generate_random_file_name()
    logging.info('Writing temporal output file: ' + second_temporal_csv)
    ...
    for user in unique_usernames:
        ...
        if granularity is not None:
            for date_period in unique_dates:
                ...
                score = compute_score(user, number_of_rts, effective_decay, dictionary_periods)
    ...
    #Create elites file
    logging.info('Writing output file: ' + outfile)
